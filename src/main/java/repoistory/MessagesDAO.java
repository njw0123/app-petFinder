...
public class MessagesDAO extends DAO {
	...
	// 특정 동물에 대한 메세지들를 읽어오는 것을 처리할 메서드
	public static List<Message> readMessage(String target) {
	...
	public static int deleteMessage(String no, String pass) {
		SqlSession session = factory.openSession(true);
		Map<String, Object> obj = new HashMap<>();
		obj.put("no", no);
		obj.put("pass", pass);
		session.delete("messages.delete", obj);
		session.close();
		return 0;
	}
}
